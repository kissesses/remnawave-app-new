...
def _is_blocked_remnawave_host(host: str) -> bool:
    ...
    try:
        for info in socket.getaddrinfo(host, None, type=socket.SOCK_STREAM):
            ...
            if ip.is_private and str(ip) not in ("127.0.0.1", "::1"):
                return True
            if ip.is_loopback and str(ip) not in ("127.0.0.1", "::1"):
                return True
...
def _validate_remnawave_base_url(base_url: str) -> str:
    url = _normalize_remnawave_base_url(base_url)
    if not url:
        raise RemnawaveAPIError("Remnawave base URL is required")
    parsed = urlparse(url if "://" in url else f"https://{url}")
    if parsed.scheme not in ("https", "http"):
        raise RemnawaveAPIError("Remnawave URL must use http or https")
    host = (parsed.hostname or "").strip()
    if not host:
        raise RemnawaveAPIError("Remnawave URL host is invalid")
    if parsed.scheme == "http" and host not in ("localhost", "127.0.0.1", "::1"):
        raise RemnawaveAPIError("HTTP Remnawave URL allowed only for localhost")
    if _is_blocked_remnawave_host(host):
        raise RemnawaveAPIError("Remnawave URL points to a forbidden network target")
    port = parsed.port
    if port is not None and port not in (80, 443, 8080, 8443):
        raise RemnawaveAPIError("Remnawave URL uses a disallowed port")
    normalized = f"{parsed.scheme}://{host}"
    if port and port not in (80, 443):
        normalized += f":{port}"
    if parsed.path and parsed.path not in ("", "/"):
        raise RemnawaveAPIError("Remnawave base URL must not include a path")
    return normalized.rstrip("/")


def _normalize_remnawave_token(token: str) -> str:
...
def _build_remnawave_headers(token: str) -> dict[str, str]:
    """Headers required by Remnawave behind reverse proxy (see docs.rw SDK examples)."""
    return {
        "Authorization": f"Bearer {_normalize_remnawave_token(token)}",
...
async def test_connection(base_url: str, token: str) -> tuple[bool, str]:
    """Validate Remnawave API URL and bearer token (JWT from API Tokens section)."""
    try:
        url_base = _validate_remnawave_base_url(base_url)
    except RemnawaveAPIError as exc:
        return False, str(exc)
    api_token = _normalize_remnawave_token(token)
    if not url_base or not api_token:
        return False, "Укажите URL и API token Remnawave"

    headers = _build_remnawave_headers(api_token)
    probe_requests: tuple[tuple[str, dict[str, Any] | None], ...] = (
    ...
    try:
        async with httpx.AsyncClient(timeout=20.0, follow_redirects=True) as client:
            ...
            for path, params in probe_requests:
                response = await client.get(f"{url_base}{path}", headers=headers, params=params)
                last_status = response.status_code
                logger.info("Remnawave probe %s -> %s", path, last_status)
                if response.status_code in (200, 404):
                    return True, "Подключение к Remnawave успешно"
                if response.status_code in (401, 403):
                    saw_auth_error = True
                    continue
            if last_status == 502:
                return False, "Remnawave недоступен (502). Проверьте URL панели — укажите https://домен без /api"
            if saw_auth_error:
                return False, (
                    "Неверный API token или недостаточно прав. "
                    "Создайте токен в Remnawave → Settings → API Tokens и укажите URL панели без /api"
                )
            return False, f"Remnawave ответил кодом {last_status}. Проверьте URL (https://домен, без /api)"
    except httpx.RequestError as exc:
        return False, f"Не удалось подключиться к Remnawave: {exc}. Проверьте URL (https://ваша-панель.ru, без /api)"


def _load_config() -> dict[str, Any]:
    """Backward-compatible global config loader (deprecated)."""
    base_url = _validate_remnawave_base_url(rw_repo.get_setting("remnawave_base_url") or "")
...
def _load_config_for_host(host_name: str) -> dict[str, Any]:
    ...
    base_url_raw = (squad.get("remnawave_base_url") or "").strip()
