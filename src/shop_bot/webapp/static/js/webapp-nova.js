(function () {
    ...
    function isActive() {
        return document.documentElement.dataset.webappDesign === 'nova';
    }
    ...
    function renderSubscriptionCard(key) {
        if (!key) {
            return `
                <article class="webapp-nv-card">
                    <div class="webapp-nv-card__head">
                        <div class="webapp-nv-card__icon"><span class="material-icons-round">inventory_2</span></div>
                        <h3 class="webapp-nv-card__title">Подписка</h3>
                    </div>
                    <div class="webapp-nv-empty">
                        <p>Нет активной подписки</p>
                        <button type="button" class="webapp-nv-btn webapp-nv-btn--primary" data-nv-action="tariffs">Выбрать тариф</button>
                    </div>
                </article>`;
        }
        ...
        const statusBadge = active
            ? '<span class="webapp-nv-badge webapp-nv-badge--active"><span class="webapp-nv-badge__dot"></span> Активна</span>'
            : '<span class="webapp-nv-badge webapp-nv-badge--muted">Истекла</span>';
        const daysBadge = active && key.days_left != null
            ? `<span class="webapp-nv-badge webapp-nv-badge--muted">${daysLabel(key.days_left)}</span>`
            : '';
        ...
        return `
            <article class="webapp-nv-card">
                ...
                <div class="webapp-nv-badges">${statusBadge}${daysBadge}</div>
                <div class="webapp-nv-rows">
                    <div class="webapp-nv-row">
                        <span class="webapp-nv-row__label">Тариф</span>
                        <span class="webapp-nv-row__value">${planName}</span>
                    </div>
                    <div class="webapp-nv-row">
                        <span class="webapp-nv-row__label">До</span>
                        <span class="webapp-nv-row__value">${formatExpireDate(key.expire_date_str)}</span>
                    </div>
                    <div class="webapp-nv-row">
                        <span class="webapp-nv-row__label">Трафик</span>
                        <span class="webapp-nv-row__value">${key.traffic_info?.includes('∞') || key.traffic_info?.endsWith('/ ∞') ? 'Безлимит' : (key.traffic_info || '—')}</span>
                    </div>
                </div>
            </article>`;
    }
    ...
    function renderBalanceCard(balance, cfg) {
        ...
        return `
            <article class="webapp-nv-card webapp-nv-card--balance">
                <div class="webapp-nv-card__head">
                    <div class="webapp-nv-card__icon"><span class="material-icons-round">account_balance_wallet</span></div>
                    <h3 class="webapp-nv-card__title">Баланс</h3>
                </div>
                <div class="webapp-nv-balance-amount">${formatMoney(balance)}</div>
                <button type="button" class="webapp-nv-btn webapp-nv-btn--primary webapp-nv-btn--block" data-nv-action="topup">
    ...
    function renderReferralCard(cfg, status) {
        ...
        if (!enabled) {
            return `
                <article class="webapp-nv-card">
                    <div class="webapp-nv-card__head">
                        <div class="webapp-nv-card__icon"><span class="material-icons-round">group</span></div>
                        <h3 class="webapp-nv-card__title">Рефералы</h3>
                    </div>
                    <p class="webapp-nv-muted">Реферальная программа отключена.</p>
                </article>`;
        }
        ...
        return `
            <article class="webapp-nv-card" id="webapp-nv-ref-card">
                <div class="webapp-nv-card__head">
                    <div class="webapp-nv-card__icon"><span class="material-icons-round">group</span></div>
                    <h3 class="webapp-nv-card__title">Рефералы</h3>
                </div>
                <p class="webapp-nv-muted">Приглашайте друзей и получайте бонусы на баланс.</p>
                <div class="webapp-nv-ref-input">
                    <code title="${link}">${link || '—'}</code>
                    <button type="button" class="webapp-nv-ref-copy" data-nv-copy-ref aria-label="Копировать">
                        <span class="material-icons-round">content_copy</span>
                    </button>
                </div>
                <button type="button" class="webapp-nv-link-btn" data-nv-action="referral-stats">
                    Статистика (${count}) →
                </button>
            </article>`;
    }
    ...
    function renderQuickActions(cfg) {
        ...
        return `
            <div class="webapp-nv-quick">
                ${items.map((item) => `
                    <button type="button" class="webapp-nv-quick__btn" data-nv-action="${item.action}">
                        <span class="webapp-nv-quick__icon"><span class="material-icons-round">${item.icon}</span></span>
                        <span class="webapp-nv-quick__label">${item.label}</span>
                    </button>
                `).join('')}
            </div>`;
    }
    ...
    function renderHero(key, status, cfg) {
        ...
        return `
            <section class="webapp-nv-hero">
                <h1 class="webapp-nv-hero__title">${heroTitle}</h1>
                <p class="webapp-nv-hero__sub">${subText}</p>
                ${hasKey && key.sub_url ? `
                    <button type="button" class="webapp-nv-btn webapp-nv-btn--primary" data-nv-action="connect">
                        <span class="material-icons-round">wifi</span> Подключиться
                    </button>
                ` : status?.trial_available && cfg?.modules?.trial !== false ? `
                    <button type="button" class="webapp-nv-btn webapp-nv-btn--primary" data-nv-action="trial">
                        <span class="material-icons-round">card_giftcard</span> Пробный период
                    </button>
                ` : `
                    <button type="button" class="webapp-nv-btn webapp-nv-btn--primary" data-nv-action="tariffs">
    ...
    function bindDashboardActions(root, data) {
        ...
        root.querySelector('[data-nv-copy-ref]')?.addEventListener('click', () => {
            if (!link) return;
            navigator.clipboard?.writeText(link).then(() => notify('Ссылка скопирована', 'success'));
        });

        root.addEventListener('click', (e) => {
            const btn = e.target.closest('[data-nv-action]');
            if (!btn) return;
    ...
    function updateHeaderBalance(balance) {
        const el = document.querySelector('[data-nv-balance-amount]');
    ...
    async function renderDashboard() {
        ...
        let root = document.getElementById('webapp-nova-dashboard');
        if (!root) {
            root = document.createElement('div');
            root.id = 'webapp-nova-dashboard';
            main.prepend(root);
        }

        root.innerHTML = '<div class="webapp-nv-loading">Загрузка…</div>';
        ...
        if (accent && /^#[0-9a-fA-F]{3,8}$/.test(accent)) {
            document.documentElement.style.setProperty('--wa-nv-accent', accent);
    ...
    function renderHeader() {
        if (document.getElementById('webapp-nova-header')) return;
        ...
        header.id = 'webapp-nova-header';
        header.className = 'webapp-nova-header';
        header.innerHTML = `
            <div class="webapp-nova-header__inner">
                <div class="webapp-nova-header__brand">
                    ${logoSrc ? `<img src="${logoSrc}" alt="" />` : '<span class="material-icons-round">shield</span>'}
                    <span>${brand}</span>
                </div>
                <div class="webapp-nova-header__actions">
                    <button type="button" class="webapp-nova-header__balance" data-nv-action="topup">
                        <span class="material-icons-round">account_balance_wallet</span>
                        <span data-nv-balance-amount>0 ₽</span>
                    </button>
                    <button type="button" class="webapp-nova-header__avatar" id="webapp-nv-profile-btn" title="Профиль">
                        ${getUserInitial()}
                    </button>
                </div>
            </div>`;
        ...
        header.addEventListener('click', (e) => {
            const topup = e.target.closest('[data-nv-action="topup"]');
            if (topup) {
                window.WebAppCabinet?.openTopUp?.();
                return;
            }
            if (e.target.closest('#webapp-nv-profile-btn')) navigate('profile-page');
        });
    ...
    function renderTabBar() {
        if (document.getElementById('webapp-nova-tabbar')) return;
        ...
        nav.id = 'webapp-nova-tabbar';
        nav.className = 'webapp-nova-tabbar';
        nav.setAttribute('aria-label', 'Навигация');
        nav.innerHTML = `
            <div class="webapp-nova-tabbar__inner">
                ${items.map((item) => `
                    <button type="button" class="webapp-nova-tabbar__btn" data-page-id="${item.id}">
                        <span class="material-icons-round">${item.icon}</span>
                        <span class="webapp-nova-tabbar__label">${item.label}</span>
                    </button>
                `).join('')}
            </div>`;
        ...
        document.body.classList.add('webapp-has-nova-tabbar');
    }
    ...
    function syncNav(pageId) {
        ...
        document.querySelectorAll('#webapp-nova-tabbar [data-page-id]').forEach((btn) => {
            btn.classList.toggle('is-active', btn.dataset.pageId === id);
        });
    }

    function destroy() {
        document.getElementById('webapp-nova-header')?.remove();
        document.getElementById('webapp-nova-tabbar')?.remove();
        document.getElementById('webapp-nova-dashboard')?.remove();
        document.body.classList.remove('webapp-has-nova-tabbar');
    }
