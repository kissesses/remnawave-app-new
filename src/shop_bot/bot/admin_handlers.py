...
from shop_bot.modules.referral_rewards import WITHDRAW_CMD_INCOMPLETE_RE, WITHDRAW_CMD_RE
...
def get_admin_router() -> Router:
    ...
    async def admin_backup_db(callback: types.CallbackQuery):
        ...
        try:
            result = await backup_manager.deliver_backup_notifications(
                callback.bot, zip_path, created.password, fallback_to_admins=False
            )
            sent = int(result.get('archive') or 0)
            secret = int(result.get('secret') or 0)
        except Exception:
            sent = 0
            secret = 0
        if sent:
            txt = f"✅ Бэкап создан: <b>{zip_path.name}</b>\nАрхив отправлен в канал"
            if secret:
                txt += ", пароль — в секретный топик"
        else:
            txt = (
                f"✅ Бэкап создан: <b>{zip_path.name}</b>\n"
                f"⚠️ В Telegram не отправлен — настройте каналы в панели (Боты → Каналы)"
            )
        if wait:
            try:
    ...
    @admin_router.message(F.text.regexp(WITHDRAW_CMD_INCOMPLETE_RE))
    async def withdraw_cmd_missing_id_handler(message: types.Message):
        if not is_admin(message.from_user.id):
            await message.answer("⛔ Команда доступна только администраторам.")
            return
        from shop_bot.modules.referral_rewards import withdraw_command_help, WITHDRAW_CMD_INCOMPLETE_RE

        m = WITHDRAW_CMD_INCOMPLETE_RE.match((message.text or "").strip())
        action = (m.group("action") if m else "approve_withdraw").lower()
        await message.answer(withdraw_command_help(action))

    @admin_router.message(F.text.regexp(WITHDRAW_CMD_RE))
    async def withdraw_command_handler(message: types.Message):
        ...
        from shop_bot.modules.referral_rewards import (
            ...
            parse_withdraw_command,
            withdraw_command_help,
        )

        parsed = parse_withdraw_command(message.text)
        if not parsed:
            await message.answer(withdraw_command_help())
            return

        action, target_user_id = parsed
        if action == "approve_withdraw":
            await _approve_withdraw(message, target_user_id, minimum_withdrawal_amount())
        else:
            await _decline_withdraw(message, target_user_id)

    async def _approve_withdraw(message: types.Message, user_id: int, min_wd: float) -> None:
        user = get_user(user_id)
        if not user:
        ...
        if balance <= 0:
            await message.answer(
                f"ℹ️ У пользователя {user_id} реф. баланс к выводу: <b>0 ₽</b>.\n"
                f"Lifetime: <b>{float(user.get('referral_balance_all') or 0):.2f} ₽</b>."
            )
            return
        if balance < min_wd:
            await message.answer(
                f"⚠️ Баланс пользователя {user_id}: <b>{balance:.2f} ₽</b> — "
                f"меньше минимума для вывода (<b>{min_wd:.0f} ₽</b>)."
            )
            return
        set_referral_balance(user_id, 0)
        await message.answer(
            f"✅ Выплата <b>{balance:.2f} ₽</b> пользователю <code>{user_id}</code> подтверждена.\n"
            f"Переведите средства вручную и сохраните чек."
        )
        try:
            await message.bot.send_message(
                user_id,
                f"✅ Ваша заявка на вывод {balance:.2f} RUB одобрена. "
                f"Деньги будут переведены в ближайшее время.",
            )
        except Exception as exc:
            await message.answer(f"⚠️ Баланс обнулён, но не удалось написать пользователю: {exc}")

    async def _decline_withdraw(message: types.Message, user_id: int) -> None:
        user = get_user(user_id)
        if not user:
            await message.answer(f"❌ Пользователь {user_id} не найден в базе.")
            return
        ...
        await message.answer(
            ...
            f"(реф. баланс не изменён: <b>{balance:.2f} ₽</b>)."
        )
        try:
            await message.bot.send_message(
                user_id,
                "❌ Ваша заявка на вывод отклонена. "
                "Проверьте корректность реквизитов и попробуйте снова.",
            )
        except Exception as exc:
            await message.answer(f"⚠️ Не удалось уведомить пользователя: {exc}")
