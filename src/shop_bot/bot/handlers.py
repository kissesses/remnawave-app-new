...
ADMIN_ID = None
CRYPTO_BOT_TOKEN = None  # lazy: см. get_cryptobot_token()


def bot_msg(template_id: str, fallback: str = "", **ctx) -> str:
    rendered = bot_messages.render(template_id, ctx)
    return rendered if rendered else fallback

logger = logging.getLogger(__name__)


from shop_bot.support.log_utils import log_suppressed as _log_suppressed
def get_cryptobot_token() -> str | None:
...
async def create_pending_payment(user_id: int, amount: float, payment_method: str, action: str, metadata_source: dict, plan_id: int = None, months: int = 0) -> str:
    ...
    if metadata_source.get("expected_amount_ton") is not None:
        metadata["expected_amount_ton"] = float(metadata_source["expected_amount_ton"])
    ...
    async def decorated_function(event: types.Update, *args, **kwargs):
        ...
        if recent_count >= 3:
            ...
            message_text = bot_msg(
                ...
                block_seconds=str(int(block_duration)),
            )
            try:
...
def get_user_router() -> Router:
    ...
    async def start_handler(message: types.Message, state: FSMContext, bot: Bot, command: CommandObject):
        ...
        if reward_type == "fixed_start_referrer" and referrer_id and user_data and not user_data.get('referral_start_bonus_received'):
            from shop_bot.modules.referral_rewards import referrals_enabled, credit_referrer, maybe_notify_referrer
            if referrals_enabled():
                ...
                if start_bonus > 0:
                    ok = credit_referrer(int(referrer_id), float(start_bonus))
                    try:
                        set_referral_start_bonus_received(user_id)
                    except Exception:
                        _log_suppressed("suppressed")
                    if ok:
                        await maybe_notify_referrer(
                            bot,
                            int(referrer_id),
                            bot_msg(
                                ...
                                (
                                    ...
                                    f"Бонус: <code>{float(start_bonus):.2f} RUB</code>"
                                ),
                                user_name=message.from_user.full_name,
                                user_id=str(user_id),
                                bonus_amount=f"{float(start_bonus):.2f} RUB",
                            ),
                        )
        ...
        if user_data and user_data.get('agreed_to_terms'):
            await message.answer(
                bot_msg(
                    "welcome_return",
                    f"👋 С возвращением, {html.bold(message.from_user.full_name)}!",
                    user_name=message.from_user.full_name,
                ),
    ...
    async def check_subscription_handler(callback: types.CallbackQuery, state: FSMContext, bot: Bot):
        ...
        try:
            ...
            else: await callback.answer(bot_msg("onboarding_not_subscribed", "❌ Вы еще не подписались на канал. Пожалуйста, подпишитесь и попробуйте снова."), show_alert=True)
        except Exception as e:
            logger.error(f"Ошибка проверки подписки (ID: {user_id}, Канал: {channel_url}): {e}")
            await callback.answer(bot_msg("onboarding_check_failed", "⚠️ Не удалось проверить подписку. Убедитесь, что бот является администратором канала."), show_alert=True)
    ...
    async def onboarding_fallback_handler(message: types.Message):
        await message.answer(bot_msg("onboarding_fallback", "⚠️ Пожалуйста, выполните требуемые действия и нажмите кнопку в сообщении выше для продолжения."))
    ...
    async def create_stars_invoice_handler(callback: types.CallbackQuery, state: FSMContext):
        ...
        if not plan:
            await smart_edit_message(callback.message, bot_msg("payment_plan_not_found", "❌ Ошибка: Тариф не найден в системе."))
            await state.clear()
            return
        ...
        if stars_ratio <= 0:
            await smart_edit_message(callback.message, bot_msg("payment_stars_unavailable", "⚠️ Оплата через Telegram Stars временно недоступна."))
            await state.clear()
            return
        ...
        try:
            ...
            await smart_edit_message(callback.message, bot_msg("payment_invoice_error", "❌ Ошибка при создании счета Stars. Попробуйте другой метод."))
            await state.clear()
    ...
    async def topup_stars_handler(callback: types.CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            await smart_edit_message(callback.message, bot_msg("payment_invoice_error", "❌ Ошибка при создании счета в Stars."))
            await state.clear()
    ...
    async def pay_yoomoney_handler(callback: types.CallbackQuery, state: FSMContext):
        ...
        if not (w.isdigit() and len(w) >= 11):
            await smart_edit_message(callback.message, bot_msg("payment_yoomoney_config", "❌ Ошибка конфигурации YooMoney. Обратитесь к администратору."))
            await state.clear()
            return
    ...
    async def topup_pay_cryptobot(callback: types.CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            price_str = f"{Decimal(str(amount)).quantize(Decimal('0.01'))}"
            payload_str = f"{payment_id}:" + ":".join([str(int(user_id)), "0", price_str, "top_up", "None", "", "None", "None", "CryptoBot", "None", "0"])
    ...
    async def topup_pay_tonconnect(callback: types.CallbackQuery, state: FSMContext):
        ...
        data["expected_amount_ton"] = float(price_ton)
    ...
    async def referral_program_handler(callback: types.CallbackQuery):
        await callback.answer()
        from shop_bot.modules.referral_rewards import referrals_enabled, render_program_extra
        if not referrals_enabled():
            await smart_edit_message(callback.message, "🚫 <b>Реферальная программа временно отключена.</b>", keyboards.create_back_to_menu_keyboard())
            return
        user_id = callback.from_user.id
    ...
    async def trial_period_handler(callback: types.CallbackQuery, state: FSMContext):
        ...
        if not hosts:
            await smart_edit_message(callback.message, bot_msg("trial_no_hosts", "😔 К сожалению, сейчас нет свободных серверов для пробного периода. Попробуйте позже."), keyboards.create_back_to_menu_keyboard())
            return
    ...
    async def process_trial_key_creation(message: types.Message, host_name: str):
        ...
        try:
            ...
            await smart_edit_message(message, bot_msg("trial_failed", "⚠️ <b>Произошла ошибка</b>\nНе удалось завершить создание пробного ключа."))
    ...
    async def create_cryptobot_invoice_handler(callback: types.CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            payload = f"{pid}:" + ":".join([str(callback.from_user.id), str(plan['months']), f"{price:.2f}", str(data['action']), str(data.get('key_id') or "None"), str(data.get('host_name') or ""), str(plan['plan_id']), str(data.get('customer_email') or "None"), "CryptoBot", str(data.get('promo_code') or "None"), f"{data.get('promo_discount', 0):.2f}", str(data.get('tier_device_count') or 'None')])
    ...
    async def check_crypto_invoice_handler(callback: types.CallbackQuery, state: FSMContext, bot: Bot):
        ...
        try:
            ...
            if len(p) >= 10 and uuid_re.match(p[0]):
                metadata = find_and_complete_pending_transaction(p[0])
                if not metadata:
                    return await callback.message.answer("⚠️ Платёж уже обработан или не найден.")
            else:
...
async def notify_admin_of_purchase(bot: Bot, metadata: dict):
    try:
        ...
        if promo:
            ...
            stats = []
            if metadata.get('promo_usage_total_limit'):
                stats.append(f"Общий: {metadata.get('promo_usage_total_used') or 0}/{metadata.get('promo_usage_total_limit')}")
            if metadata.get('promo_usage_per_user_limit'):
                stats.append(f"На юзера: {metadata.get('promo_usage_per_user_used') or 0}/{metadata.get('promo_usage_per_user_limit')}")
            if stats:
                promo_block += "\n📊 " + " | ".join(stats)
        ...
        txt = bot_msg(
            ...
            action_label='Новый ключ ➕' if action == 'new' else 'Продление ♻️',
            today_rub=f"{today_rub:,.2f}",
            today_crypto=f"{today_crypto:,.2f}",
    ...
    try:
        ...
        if action == "top_up":
            ...
            if (pay_method or '').lower() != 'balance':
                from shop_bot.modules.referral_rewards import (
                    referrals_enabled,
                    compute_purchase_reward,
                    credit_referrer,
                    maybe_notify_referrer,
                    payout_mode,
                )
                if referrals_enabled():
                    ...
                    if ref_id:
                        ...
                        reward = compute_purchase_reward(
                            price,
                            seller_ref_percent if seller_ref_percent > 0 else None,
                        )

                        if float(reward) > 0:
                            if payout_mode() == 'referral_balance':
                                ref_old_balance = get_referral_balance(int(ref_id))
                            else:
                                ref_old_balance = get_balance(int(ref_id))
                            if credit_referrer(int(ref_id), float(reward)):
                                ref_new_balance = (
                                    get_referral_balance(int(ref_id))
                                    if payout_mode() == 'referral_balance'
                                    else get_balance(int(ref_id))
                                )
            ...
            if not str(uid).startswith("999"):
                try:
                    if bot: await bot.send_message(
                        uid,
                        bot_msg(
                            "topup_success",
                            f"✅ <b>Баланс пополнен!</b>\nСумма: <code>{float(price):.2f} RUB</code>\nТекущий баланс: <code>{balance:.2f} RUB</code>",
                            amount=f"{float(price):.2f} RUB",
            ...
            try:
                ...
                await asyncio.to_thread(smtp_mailer.try_send_payment_receipt_for_metadata, topup_meta)
        ...
        if not str(uid).startswith("999"):
            try:
                if bot: proc_msg = await bot.send_message(
                    uid,
                    bot_msg(
                        "payment_processing",
        ...
        try:
            if action == "new":
                ...
                if not key:
                    ...
                    if proc_msg and bot: await proc_msg.edit_text(bot_msg("payment_key_not_found", "❌ Ключ для продления не найден."))
                    return False
            ...
            try:
                ...
                await asyncio.to_thread(
                    smtp_mailer.try_send_payment_receipt_for_metadata,
                    metadata,
                    expiry_str=exp.strftime('%d.%m.%Y %H:%M'),
                )
            except Exception as exc:
                logger.warning("Payment receipt email error: %s", exc)
