...
async def send_subscription_notification(bot: Bot, user_id: int, key_id: int, time_left_hours: int, expiry_date: datetime):
    try:
        ...
        message = bot_messages.render("subscription_expiry", {
            "time_left": time_text,
...
async def send_subscription_email_notification(
    user_id: int,
    key_id: int,
    time_left_hours: int,
    ...
    try:
        ...
        from shop_bot.data_manager.remnawave_repository import get_user

        if not smtp_mailer.key_expiry_enabled():
            return
        user = get_user(user_id) or {}
        to_email = (user.get("auth_email") or "").strip().lower()
        if not to_email:
            return
        time_text = format_time_left(time_left_hours)
        expiry_str = expiry_date.strftime('%d.%m.%Y в %H:%M')
        ok, err = smtp_mailer.send_key_expiry_reminder(
            to_email,
            time_left_label=time_text,
            expiry_str=expiry_str,
            host_name=host_name,
        )
        if ok:
            logger.debug(
                "Scheduler: Email expiry reminder sent to user %s key %s (%s h left)",
                user_id, key_id, time_left_hours,
            )
        else:
            logger.warning("Scheduler: Email expiry reminder failed for user %s: %s", user_id, err)
    except Exception as e:
        logger.error("Scheduler: Email expiry notification error for user %s: %s", user_id, e)

def _cleanup_notified_users(all_db_keys: list[dict]):
    if not notified_users and not email_notified_users:
        return
    ...
    for cache in (notified_users, email_notified_users):
        users_to_check = list(cache.keys())
        for user_id in users_to_check:
            keys_to_check = list(cache[user_id].keys())
            for key_id in keys_to_check:
                if key_id not in active_key_ids:
                    del cache[user_id][key_id]
        ...
        try:
            ...
            for hours_mark in NOTIFY_BEFORE_HOURS:
                if hours_mark - 1 < total_hours_left <= hours_mark:
                    ...
                    email_notified_users.setdefault(user_id, {}).setdefault(key_id, set())

                    if hours_mark not in notified_users[user_id][key_id]:
                        if bot and not str(user_id).startswith("999"):
                    ...
                    if hours_mark not in email_notified_users[user_id][key_id]:
                        await send_subscription_email_notification(
                            user_id,
                            key_id,
                            hours_mark,
                            expiry_date,
                        ...
                        email_notified_users[user_id][key_id].add(hours_mark)
                    break
...
async def _maybe_run_daily_backup(bot: Bot):
    """Автобэкап БД по интервалу backup_interval_days; опционально Telegram и ротация."""
    ...
    if _last_backup_run_at and (now - _last_backup_run_at).total_seconds() < interval_seconds:
        return
    if cfg.get("autobackup_telegram") and cfg.get("autobackup_delivery_blocked"):
        for msg in cfg.get("delivery_alerts") or []:
            logger.warning("Scheduler: автобэкап пропущен — %s", msg)
        return
    ...
    try:
        ...
        if zip_path and zip_path.exists():
            if cfg.get("autobackup_telegram"):
                try:
                    result = await backup_manager.deliver_backup_notifications(
                        bot, zip_path, created.password, fallback_to_admins=False
                    )
                    logger.info(
                        "Scheduler: бэкап %s → archive=%s secret=%s",
                        zip_path.name, result.get('archive'), result.get('secret'),
                    )
                    if not result.get("archive"):
                        err = result.get("archive_error") or "канал не настроен или ошибка Telegram"
                        logger.error("Scheduler: архив не доставлен в Telegram: %s", err)
                except Exception as e:
                    logger.error("Scheduler: не удалось отправить бэкап: %s", e)
            else:
                logger.info("Scheduler: создан автобэкап %s", zip_path.name)
            try:
                backup_manager.cleanup_old_backups(keep=cfg.get("keep_count"))
            except Exception:
                pass
        _last_backup_run_at = now
    except Exception as e:
        logger.error("Scheduler: ошибка автобэкапа: %s", e, exc_info=True)
...
async def _send_alert(bot: Bot, scope: str, name: str, issues: list[dict], level: str):
    ...
    try:
        from shop_bot.data_manager import smtp_mailer
        await asyncio.to_thread(smtp_mailer.send_monitoring_alert, scope, name, issues, level)
    except Exception:
        logger.debug("SMTP monitoring alert failed", exc_info=True)
